...
import me.ninethousand.ninehack.mixin.mixins.game.MixinBlock;
...
public class InventoryUtil implements NineHack.Globals {
    ...
    public static List<Integer> findEmptySlots(boolean withXCarry) {
        ...
        for (Map.Entry<Integer, ItemStack> entry : InventoryUtil.getInventoryAndHotbarSlots().entrySet()) {
            ...
            if (!(isEmpty) && entry.getValue().getItem() != Items.AIR) continue;
        ...
        switch (mode) {
            ...
            case SILENT: {
                if (!back && !switchedItem) {
                    ...
                    switchedItemSwitched[0] = false;
                    NineHack.INVENTORY_MANAGER.recoverSilent(lastHotbarSlot);
                }
                switchedItemSwitched[1] = true;
                break;
            }
